...
function MyComponent() {    
    const [text, setText] = React.useState(['this'])
    return (
      <div onClick={() => setText((previous) => [previous, ...[' and this']])}>
        <p>Just like {text.map(value => <span>{value}</span>)} </p>
      </div>
    );
  }
...
``` javascript
import './style.css'; // We are adding styles here
function MyStyledComponent() {    
    const [text, setText] = React.useState([])
    return (
      <div onClick={() => setText((previous) => [...previous, ' and this one'])}>
        <div className='styled-component-container'>
          <span className='wrapper-for-other-component'>This one is styled</span> 
...
- Changes to your component will not refresh quarto, `quarto preview` will need to be called.
